...
std::pair<size_t, bool>  WeightUtils::computeExpectedWeights(const Architecture &arch)
{
    size_t total = 0;
    ...
    for (const Layer &layer : arch.getLayers())
    {
        ...
        case LayerType::LAYER_CONV2D:
        {
            ...
            {
                Logger::log(LogLevel::ERROR, "Invalid CONV2D layer while computing weights");
                return {0, false};
            }
        ...
        case LayerType::LAYER_DENSE:
        {
            ...
            {
                Logger::log(LogLevel::ERROR, "Invalid DENSE layer while computing weights");
                return {0, false};
            }
        ...
        case LayerType::LAYER_BATCHNORM:
        {
            ...
            if (channels <= 0)
            {
                Logger::log(LogLevel::ERROR, "Invalid BATCHNORM layer while computing weights");
                return {0, false};
            }
        ...
        default:
            Logger::log(LogLevel::ERROR, "Unknown layer type while computing weights");
            return {0, false};
        }
    }
...
bool WeightUtils::validateWeights(
    const Architecture& arch,
    const std::vector<float>& weights)
{

    auto expected = computeExpectedWeights(arch); 
    std::cout<< expected.first << std::endl; 
    if (!expected.second)
    {
        Logger::log(LogLevel::ERROR,
            "[WeightUtils] Failed to compute expected weights");
        return false;
    }

    if (weights.size() != expected.first)
    {
        Logger::log(LogLevel::WARNING,
            "[WeightUtils] Weight size mismatch. Expected: " +
